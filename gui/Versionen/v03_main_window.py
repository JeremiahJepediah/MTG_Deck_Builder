...
class MainWindow:
    ...
    def setup_random_cards_column(self, parent):
        """Spalte 1: Zufällige Karten / Suche"""
        tk.Label(parent, text="Karten", font=('Arial', 12, 'bold')).pack(pady=5)
    ...
    def do_search(self):
        ...
        if not search_term:
            return  # Nichts tun, Nutzer kann "Zufällig" klicken
        ...
        for card in results[:100]:  # Max 100 Ergebnisse
            display = f"{card['name']}"
            self.random_listbox.insert('end', display)
            self.random_card_data[display] = card['id']
    
    def clear_search(self):
        """Leere Suchfeld und zeige wieder zufällige Karten"""
